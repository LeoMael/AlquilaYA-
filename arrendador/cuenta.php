...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['actualizar_cuenta'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if (password_verify($password_actual, $arrendador['password'])) {
            if ($password_nueva != $password_confirmar) {
                ...
                $sql_update = "UPDATE arrendadores SET email = ?, password = ? WHERE id_arrendador = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("ssi", $email, $password_nueva_hash, $id_arrendador);
